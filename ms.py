...
#key generation: generate system parameter a and key pairs ((s1[i], s2[i]), u[i]), i=0, 1, 2 
 ...
 start_time=time.time()  # record the protocol starting time
...
#Round R-3:
  #first, check if r[i]=Hash(0|v[i]|u[i])
 for i in range(3):
...
#check if bar_v[1]+...bar_v[mu]=a*bar_z1+bar_z2-bar_u*c
 ...
 if LT!=RT:
     ...
     #print("final eta_t={}".format(eta_t))
 else:
 ...
 print("runtime is {} seconds".format(runtime))
