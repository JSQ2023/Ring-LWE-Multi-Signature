...
sigma=pow(n, 0.75)
q = 2475880078570760549798259707 # p is a safe prime and p=2^{91}+11259
coef_c_bound = ceil(log2(n))
deg_c_bound = n/2
coef_y_bound = ceil(pow(n, 1.5)*sigma*pow(log2(n), 3))
coef_z_bound = ceil((n-1)*pow(n, 0.5)*sigma*pow(log2(n), 3))
eta_t=5*sigma*pow(n, 2)*sqrt(t*mu)*pow(log2(n), 6)/100  #multiplied by factor 1/100 and verification still passes
